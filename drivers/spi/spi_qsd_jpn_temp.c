...
static void __init msm_spi_calculate_fifo_size(struct msm_spi *dd)
{
	...
	dd->input_burst_size = dd->input_block_size;
	dd->output_burst_size = dd->output_block_size;
	printk(KERN_ERR "Input block size %d Ouput Block size %d \n", dd->input_burst_size, dd->output_burst_size);
	//NEO END
...
static void msm_spi_setup_dm_transfer(struct msm_spi *dd)
{
	...
	rx_num_rows = bytes_to_send / dd->input_burst_size; //bytes to send == bytes to rec
	//NEO END
	...
	if (tx_num_rows) {
		...
		box->src_dst_len = (dd->output_burst_size << 16) | dd->output_burst_size;
	...
	if(rx_num_rows){
		...
		box->src_dst_len = (dd->input_burst_size << 16) | dd->input_burst_size;
	...
	if (!dd->rx_unaligned_len) {
		...
		//u32 rx_offset = dd->cur_transfer->len - dd->rx_unaligned_len;
		dd->rx_dmov_cmd->box.cmd &= ~CMD_LC;
...
//NEO START
static inline u32 get_chunk_size(struct msm_spi *dd, int input_burst_size, int output_burst_size)
{
	u32 cache_line = dma_get_cache_alignment();
	int burst_size = 0;

	burst_size = (input_burst_size > output_burst_size) ? input_burst_size: output_burst_size;
...
static void msm_spi_teardown_dma(struct msm_spi *dd)
{
	...
	//NEO START
	dma_free_coherent(NULL, get_chunk_size(dd, dd->input_burst_size, dd->output_burst_size), dd->tx_dmov_cmd,
	//NEO END
			  dd->tx_dmov_cmd_dma);
...
static __init int msm_spi_init_dma(struct msm_spi *dd)
{
	...
	//NEO START
	dd->tx_dmov_cmd = dma_alloc_coherent(NULL, get_chunk_size(dd, dd->input_burst_size, dd->output_burst_size),
	//NEO END
	...
	dd->rx_padding = (u8 *)ALIGN((size_t)(dd->tx_padding + dd->output_burst_size),
				     cache_line);
